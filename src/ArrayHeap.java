...
public class ArrayHeap<T> {
    ...
    private void bubbleDown(int index) {
        if (getNode(getLeftOf(index)) != null && getNode(getRightOf(index)) != null) {
            boolean cond1 = getNode(index).priority() > getNode(getLeftOf(index)).priority();
            boolean cond2 = getNode(index).priority() > getNode(getRightOf(index)).priority();
            if (cond1 && cond2) {
                ...
                } else if (getNode(getLeftOf(index)).priority() < getNode(getRightOf(index)).priority()) {
    ...
    public T removeMin() {
        ...
        double min = getNode(1).priority();
        int ind = 1;
        for (int i = 2; i < size() + 1; i++) {
            if (getNode(i).priority() < min) {
                min = getNode(i).priority();
                ind = i;
            }
        }
        swap(ind, size());
        Node out = getNode(size());
        removeNode(size());
        bubbleDown(1);
        return out.item();
    }
