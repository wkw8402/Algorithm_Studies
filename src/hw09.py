...
def roman_numerals(text):
    ...
    []
    """
    return re.findall(r'\b([IVXLCDM]+)\b', text) #+ for at least one letter long
...
def cs_classes(post):
    ...
    True
    """
    return bool(re.search('[cC][sS]\s?\d+[abcABC]*', post)) #c,s and C,S for case insensitive. ? for potential whitespace. + for at least one digit ofnumber, and * for optional ending with a,A,b,B,c,C
...
def match_time(text):
    ...
    ['2:00']
    """
    return re.findall(r'\b(2[0123]:[012345][0-9][aApP]?[mM]?|[01]?[0-9]:[012345][0-9][aApP]?[mM]?)\b', text) #separated for 2n hours and n and 1n hours because there are only 20, 21, 22 and 23 hours available for clock.
...
def area_codes(text):
    ...
    []
    """
    return re.findall(r'\b\(?(\d\d\d)\)?[\-\s]?\d\d\d[\-\s]?\d\d\d\d\b', text) #parentheses surrounding the first three digits to only capture the area codes
...
def most_common_code(text):
    ...
    """
    "*** YOUR CODE HERE ***"
    return max(area_codes(text), key = area_codes(text).count) #as previously coded, area_code(text) returns a list of area codes. so just ouput the element in area_code(text) that has the greatest count in that list 
